...
**Global integrations** (cursor, copilot-cli, antigravity) install once and cover every repo on the machine. **Per-repo integrations** (copilot, antigravity-repo, zed) must be run from inside each repo where you want them.

## Auto-integrate everything

```bash
./install.sh --integrate=all       # cursor + copilot + copilot-cli + antigravity + zed
