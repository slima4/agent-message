...
- **Use case:** N agents, one task. You to `api` agent: _"let `web` know the auth schema changed — token is nullable now"_. It runs `msg send web "schema: token nullable"`. `web`'s agent picks it up on next `msg` check, adapts its code. Like Slack between agents — plain JSONL, no human relay.
...
msg compact                 # idempotent own-log dedup + id backfill
```
...
- `AGENT_MESSAGE_DIR` — message directory. Default `${XDG_STATE_HOME:-$HOME/.local/state}/agent-message` ([XDG Base Directory](https://specifications.freedesktop.org/basedir-spec/latest/) state location — append-only logs are textbook XDG state). Honored by both the slash commands and the `msg` shell function. Files inside: `log-<alias>.jsonl` (one per writer), `.seen-<reader>` (watermark: clock-capped last-seen ts + shown ids), `.mtime-<reader>` (mtime short-circuit cache).
